...
# overwrite by https://github.com/pytorch/vision/blob/main/torchvision/models/resnet.py
...
def resnet18_en(example_input,args):
    '''
    return resnet18 as a encoder  and out_put channels

    input = tensor:[B,C,W,H]
    output = tensor:[B,C]
    '''

    model = ResNet(in_channels= example_input.shape[1],
                   layers=[2, 2, 2, 2])
    
    out_channels = model(example_input).shape[-1]
    return model,out_channels



def resnet32_en(example_input,args):
    '''
    return resnet18 as a encoder  and out_put channels
